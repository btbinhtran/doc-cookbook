 ...
 */
var templatePath = exports.templatePath = '../../templates/tower/docs.ejs';
 ...
 */
exports.render = function(towerModules, options){
    ...
    return ejs.compile(template, { filename:templatePath })(towerModules);
};
...
exports.create = function(recipe, args, fn){
  ...
  var options = parseArgs(args),
      towerPath = options.outputDirectory, // Path to tower directory of submodules
      docPath = path.join(options.outputDirectory, 'docs'),
      packagePath = path.join(towerPath, 'package.json'),
      componentPath = path.join(towerPath, 'component.json'),
      moduleData = fs.existsSync(packagePath) ? require(packagePath)
        : fs.existsSync(componentPath) ? require(componentPath)
  ...
  var towerModules = ["style-directive", "element", "component-cookbook", "cli", "list-directive", "ec2-adapter", "mongodb-cookbook", "neo4j-cookbook", "ec2-cookbook", "ssh-cookbook", "workstation-cookbook", "expression", "stream", "cookbook", "recipe", "server-router", "validator", "type", "topology", "todomvc", "text", "strcase", "server", "run-loop", "route", "resource", "query", "program", "param", "operator", "mongodb-adapter", "memory-adapter", "load", "keyboard-directive", "fs", "emitter", "directive", "content", "console", "collection", "client-router", "attr", "adapter", "uuid", "template", "style-guide", "relation", "proxy", "asset", "s3-cookbook", "input-directive", "accessor", "filter", "rest-adapter", "cassandra-cookbook", "app-cookbook", "node-cookbook", "server-template", "reload", "client-session", "client-socket", "server-socket", "bundler", "builder", "orientdb-cookbook", "redis-cookbook", "facebook-adapter", "couchdb-adapter", "mysql-adapter", "postgres-adapter"];
  ...
  towerModules.forEach(function(module){
    ...
    // HACK: Don't git clone
    commandStr = '';

    var execCB = function(error, stdout, stderr){
      if(error) {
        console.log('clone failed' + error.toString());
